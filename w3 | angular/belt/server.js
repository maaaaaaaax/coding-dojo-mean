...
// retrieve all pets for home component
  ...
  Pet.find({}, function(err, pets){
    if(err){
       ...
       res.json({message: "Error", error: err})
        ...
        // respond with JSON
       console.log("Successfully retrieved pets: " + pets)
...
// delete a pet using the delete button on home component
...
app.post('/pets', function(req, res) {
  ...
  pet.save(function(err) {
    ...
    if(err) {
      ...
      res.json({message: "successfully added a new pet"});
    }
  })
})
...
app.get('/pets/:id', function(req, res){
  ...
  Pet.findOne({_id:id}, function(err, pet){
    if(err){
      ...
      if(pet===null){
        console.log(pet);
        res.json({message: "Author not found", data: pet});
      }
      else{
        console.log(pet);
...
app.post('/pets/upvote/:pet_id', function (req, res){
  ...
  Pet.update({_id:id}, {$inc: {"likes": 1}}, { runValidators: true }, function(err, pet){
    if(err){
      console.log("ya f***d up")
      res.json({message: "Could not upvote comment in NodeJS upvote function", error: err});
    }
    else {
      console.log("you're crushing it",pet)
      res.json({message: "Successfully upvoted post.", data: pet})
    }
  });
});
